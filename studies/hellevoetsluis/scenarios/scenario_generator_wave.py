    ...
    for demand_type in config['demand_types']:
        for sim_env in config['sim_env']:
            for wait_time in config['max_wait_times']:
                for w_wt in config['w_wt']:
                    for w_tt in config['w_tt']:
                            ...
                            for a in config['demand_scalars']:
                                for n_vehicles in config['n_vehicles']:
                                    for i in range(config['n_simulations']):
                                        ...
                                        objective = f"{config['objectives'][0]};w_wt:{w_wt};w_tt:{w_tt}"
                                        obj_str = f"WT{w_wt}TT{w_tt}"
                                        if config['objectives'][0] == 'func_key:distance_and_weighted_user_times':
                                            obj_str = "D" + obj_str
                                        # if config.get('op_max_detour_time_factor'):
                                        #     obj_str += f"DT{config['op_max_detour_time_factor']}"
                                        ...
                                        if demand_type == "reservation":
                                            ...
                                            d['op_max_detour_time_factor'] = 100000000000
                                            # d['op_reservation_module'] = "RollingHorizon"
                                            # d['op_rh_reservation_max_routes'] = 40
...
if __name__ == "__main__":
    create_scenarios('line91_button_tipping_point_boom')
